...
func channel(schema : String, table : String = "", col_value : String = "") -> RealtimeChannel:
	...
	var _channel : RealtimeChannel = get_channel(topic)
	if _channel == null:
		_channel = RealtimeChannel.new(topic, self)
		_add_channel(_channel)
	return _channel
...
func _add_channel(_channel : RealtimeChannel) -> void:
	channels.append(_channel)


func _remove_channel(_channel : RealtimeChannel) -> void:
...
func _on_data(data : Dictionary) -> void:
	match data.event:
		...
		SupabaseEvents.DELETE, SupabaseEvents.INSERT, SupabaseEvents.UPDATE:
			...
			var _channel : RealtimeChannel = get_channel(data.topic)
			if _channel != null:
				_channel._publish(data)


func get_channel(topic : String) -> RealtimeChannel:
	for _channel in channels:
		if _channel.topic == topic:
			return _channel
	return null
